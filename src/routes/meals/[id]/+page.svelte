<script lang="ts">
	...
	let mealInput = $state(data.entry.name);
	let mealDateTime = $state(dtStr(data.entry.loggedAt));
	let mealType = $state<MealType>(data.entry.mealType as MealType);
...
<PageShell title="Edit Meal" back="/meals" mainClass="px-4 py-4 pb-36">
	...
	>
		...
		{#if data.entry.recipeId}
			<a href="/recipes/{data.entry.recipeId}"
