<script lang="ts">
	...
	// Edit sheet state
	let editTarget = $state<{ id: string; name: string; ttlDays: number } | null>(null);
	let editName = $state('');
	let editTtl = $state(30);
	let editEl = $state<HTMLInputElement | undefined>(undefined);

	function openEdit(cat: PageData['categories'][0]) {
		editTarget = { id: cat.id, name: cat.name, ttlDays: cat.ttlDays };
		editName = cat.name;
		editTtl = cat.ttlDays;
...
<BottomSheet open={editTarget !== null} onclose={() => (editTarget = null)}>
	<form method="POST" action="?/update" use:enhance={() => async ({ update }) => {
		...
		<h2 class="mb-4 text-base font-semibold text-stone-900">Edit category</h2>
